/* _PDCLIB_print( const char *, struct print_status_t * )
...
static inline bool cbout(
    struct print_status_t * status,
    const void * buf,
    size_t size )
{
    size_t rv = status->write( status->pass, (char *)buf, size );
    status->count   += rv;
    status->current += rv;
    return rv == size;
}
...
static bool int2base( uintmax_t value, struct print_status_t * status )
{
    char sign = 0;
...
static bool printstr( const char * str, struct print_status_t * status )
{
...
static bool printchar( char chr, struct print_status_t * status )
{
...
extern "C"
int read_formatted( const char * spec, struct print_status_t * status )
{
