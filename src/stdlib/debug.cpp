...
namespace std {
	...
	void write(int attrib,const char *s) {
		while(*s) {
			write_c(attrib,*(s++));
		}
	}
	void debug(debug_level level,const char *s) {
		if(level<=dl) {
			write((level==dl_error?0xD:0xF),s);
		}
	}
	...
	void cls() {
		memset16(reinterpret_cast<uint16_t *>(0xC00B8000),' '|0xF,80*25);
	}
	/*
