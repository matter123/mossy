...
void sorted_insert(malloc_header *insert) {
	malloc_header *right, *left=head;
	right=left->sorted_next;
	//we want to loop till left is less than or equal to insert and
	//right is nullptr or greater than or equal to insert
	while(left->size<=insert->size&&right!=nullptr&&right->size<=insert->size) {
		left=right;
		right=right->sorted_next;
	}
	left->sorted_next=insert;
	insert->sorted_next=right;
}
