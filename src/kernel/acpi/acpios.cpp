...
#define SET(a, i) \
	do { a[(i) / 8] |= (1 << ((i) % 8)); } while(0)
#define UN_SET(a, i) \
	do { a[(i) / 8] &= ~(1 << ((i) % 8)); } while(0)
// GOOD
ACPI_STATUS AcpiOsInitialize() {
	if(initalized) { return AE_OK; }
	hal::mem_region *reg = nullptr;
	find_memregion(reg, hal::virtmem, reg->type.acpi_firmware, "[ACPIOS]");
	start = reg->start;
	len = reg->end - start;
...
ACPI_PHYSICAL_ADDRESS AcpiOsGetRootPointer() {
	Log(LOG_DEBUG, "[ACPIOS]", "obtaining RSDP");
...
void *AcpiOsMapMemory(ACPI_PHYSICAL_ADDRESS phys, ACPI_SIZE length) {
	...
	for(size_t s = 0; s < page_count - (pa_len - 1); s++) {
		for(size_t i = 0; i < pa_len; i++) {
			if(IS_SET(bitmap, s + i)) { goto skip; }
		}
		for(size_t i = 0; i < pa_len; i++) {
			SET(bitmap, s + i);
...
void AcpiOsUnmapMemory(void *where, ACPI_SIZE length) {
	...
	uintptr_t pa_len =
	    (length + (0x1000 - length % 0x1000)) / 0x1000 + (((uintptr_t)where) % 0x1000 + length % 0x1000) / 0x1000;
	for(size_t i = 0; i < pa_len; i++) {
		unmap(virt);
		UN_SET(bitmap, s + i);
	}
...
// GOOD
void *AcpiOsAllocate(ACPI_SIZE size) { return malloc((size_t)size); }
// GOOD
...
// STUB
void AcpiOsSleep(UINT64 ms) { AcpiOsStall(ms * 1000); }
// HALF STUB
...
// GOOD
void AcpiOsReleaseLock(ACPI_SPINLOCK Handle, ACPI_CPU_FLAGS Flags) { ((spinlock *)Handle)->release(); }
// GOOD
ACPI_STATUS AcpiOsInstallInterruptHandler(UINT32 InterruptLevel, ACPI_OSD_HANDLER Handler, void *Context) {
	install_handler(InterruptLevel, (void (*)(void *))Handler, Context);
	return AE_OK;
}
// STUB
ACPI_STATUS AcpiOsRemoveInterruptHandler(UINT32 InterruptLevel, ACPI_OSD_HANDLER Handler) { return AE_BAD_VALUE; }
// GOOD
...
ACPI_STATUS AcpiOsSignal(UINT32 Function, void *Info) {
	if(Function == ACPI_SIGNAL_FATAL) {
		...
		Log(LOG_ERROR, "[ACPICA]", "Fatal Signal: type %d, code %d, argument %d", info->Type, info->Code,
		    info->Argument);
...
// STUB
UINT64 AcpiOsGetTimer() { return 0; }
