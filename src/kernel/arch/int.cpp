...
void set_entry(int int_num, idt_entry *entry) { memcpy(&IDT[int_num], entry, sizeof(idt_entry)); }
...
void install_JT1(int int_num, void *target) { jump_table[int_num] = (uintptr_t)target; }
void install_JT2(int int_num, void *target) { jump_table2[int_num] = (uintptr_t)target; }
...
void def_handler(cpu_state *s) {
	if(s->int_num < EXCEPTION_COUNT) {
		...
		if(s->cs & 0x3) {
			...
			// in kernel space
			if(info->kinterrupt) {
				Log(LOG_ERROR, "[INTRPT]", "intrrupt trying to be handled whilst in interrupt old=%s new=%s",
				    info->kinterrupt->dbg, s->dbg);
			}
			info->kinterrupt = s;
		}
		for(int i = 0; i < 8; i++) {
			interrupt_handler handle = handlers[s->int_num - 32][i];
			if(handle.handler) { handle.handler(handle.context); }
		}
		if(s->cs & 0x3) {
			info->userspace = nullptr;
		} else {
			if(info->kinterrupt != s) { Log(LOG_ERROR, "[INTRPT]", "interrupt returning from unexpected spot"); }
...
void install_exception(int int_num, void (*handler)(cpu_state *)) { exceptions[int_num] = handler; }
