...
symmatch=re.compile("(([0123456789abcdef]){8}){1,2} [g|l] *[F|0]? *.text");
argv=sys.argv
argc=len(argv)
six_4_bit=0
if(argc==1):
...
if(argv[1]=="64BIT"):
	...
	if(argc!=3):
		...
		exit(0)
	infile=argv[2]
out = subprocess.Popen(["objdump","-t",infile], stdout=subprocess.PIPE).communicate()[0]
count=0
strlist=[]
...
if count==0:
	...
	exit(1)
export=open(os.path.splitext(infile)[0] + ".sym","wb")
...
for l in strlist:
	...
	if(six_4_bit):
		writeU32(export,int(l[8:15],16))
	demangle=subprocess.Popen(["c++filt","-p",l.split('.text')[1].strip().split(' ')[1]],
		stdout=subprocess.PIPE).communicate()[0].strip()[0:39]
	export.write(demangle)
	for x in range(len(demangle),40):
		writeU8(export,0);
writeU32(export,0xFFFFFFFF)
subprocess.Popen(["strip",infile], stdout=subprocess.PIPE)
